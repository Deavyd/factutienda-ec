...
def listar(db: Session = Depends(get_db), user: Usuario = Depends(get_current_user)):
    return db.query(Persona).filter(Persona.empresa_id == user.empresa_id, Persona.tipo.in_(["proveedor", "ambos"])).order_by(Persona.razon_social.asc()).all()
...
def crear(payload: PersonaCreate, db: Session = Depends(get_db), user: Usuario = Depends(require_admin)):
    ...
    data.setdefault("tipo", "proveedor")
    p = Persona(**data)
    db.add(p)
    db.commit()
    db.refresh(p)
